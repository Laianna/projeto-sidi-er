...
tokenizer = AutoTokenizer.from_pretrained('dlb/electra-base-portuguese-uncased-brwac', do_lower_case = False)

#######################INICIOS FUNÇÕES DE APOIO#######################

def map_example_to_dict(input_ids, attention_masks, label):#token_type_ids, label):
  return {
      "input_ids": input_ids,
...
def pipeline_bert(name, X_train, y_train, X_valid, y_valid, X_test, y_test): #X_train = [titulos1, titulos2]
    ...
    model = TFAutoModelForSequenceClassification.from_pretrained('dlb/electra-base-portuguese-uncased-brwac', from_pt = True)
